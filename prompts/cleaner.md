...
You will receive a batch of consecutive pages. Your task has TWO steps for each page:
...
**Step 2 — Generate `body_md`:**
Mentally classify each block as header / footer / body / footnote using the font size guidelines above, then produce clean Markdown from the body blocks only. Do not output the classification — reason through it internally.
...
- Exclude all header, footer, and footnote content
...
- body_md must contain only body content — exclude headers, footers, and footnotes
- Remove all footnote reference markers from body_md (e.g. `[1]`, `*`, `†`, superscript numbers)
