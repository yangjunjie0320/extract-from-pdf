...
_METADATA_WITH_PAGE = re.compile(
    r"^`Original Name:\s*([^`]+?),\s*Position:\s*Page\s*([^`\s]+)`$", re.MULTILINE
)
# `Original Name: Foo`  (no page)
_METADATA_NO_PAGE = re.compile(r"^`Original Name:\s*([^`]+?)`$", re.MULTILINE)
...
def write_latex(
    ...
    md = _METADATA_WITH_PAGE.sub(
        lambda m: f"\\noindent{{\\small 原标题：{m.group(1)}，原书第 {m.group(2)} 页}}\n",
        md,
    )
    md = _METADATA_NO_PAGE.sub(
        lambda m: f"\\noindent{{\\small 原标题：{m.group(1)}}}\n", md
    )
